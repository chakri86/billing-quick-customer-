...
import com.quickcustomer.billing.BuildConfig
...
import com.quickcustomer.billing.data.ExpenseStatus
...
private fun SettingsScreen(viewModel: BillingViewModel) {
        ...
        OutlinedCard(Modifier.fillMaxWidth()) {
            Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                ...
                FlowRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    OutlinedButton(onClick = { confirmEraseAllData = true }) {
    ...
    if (confirmEraseAllData) {
        AlertDialog(
            onDismissRequest = { confirmEraseAllData = false },
            title = { Text("Erase all local data?") },
            text = {
                Text(
                    "This permanently removes every user, bill, product change, expense, inventory record, setting, and saved QR image from this device. This cannot be undone."
                )
            },
            confirmButton = {
                Button(
                    onClick = {
                        confirmEraseAllData = false
                        context.getSystemService(ActivityManager::class.java).clearApplicationUserData()
                    }
                ) { Text("Erase everything") }
            },
            dismissButton = {
                TextButton(onClick = { confirmEraseAllData = false }) { Text("Cancel") }
            }
        )
    }
}
...
private const val PRIVACY_POLICY_URL =
    "https://github.com/chakri86/billing-quick-customer-/blob/main/PRIVACY_POLICY.md"
